...
Пошук страв в процессі розробки
'DELETE' поки не робив (але вже в курсі як)
Кошик не робив
Статус замовлення поки не робив
ID користувачив наче фіксується (session працює)
...
app.add_url_rule('/menu/search', view_func=search, methods=['GET', 'POST'])  # in progress
...
app.add_url_rule('/admin/search', view_func=admin_search, methods=['GET'])   # in progress
